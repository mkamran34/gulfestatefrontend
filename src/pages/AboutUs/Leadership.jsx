...
const Leadership = () => {
  ...
  useEffect(() => {
    axiosInstance.get("/api/team-members")
      .then(res => {
  ...
  >
    <img
      src={`http://127.0.0.1:8000/storage/${member.photo}`} // Assuming you're using Laravel's storage path
      alt={`${member.first_name} ${member.last_name}`}
      className="w-[90%] h-auto object-cover mb-4"
    />
    <div className="flex flex-col ml-[5%]">
      <h3 className="text-4xl font-bebas mb-2">
        {member.first_name} {member.last_name}
      </h3>
      <p className="text-gray-600 text-xl">{member.position}</p>
    </div>
  </div>
))}
